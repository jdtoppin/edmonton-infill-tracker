...
Node.js, Caddy, and PostgreSQL image pins are checked weekly by the
...
custom PostGIS, or pinned Caddy images. The scanner action is commit-pinned and does not receive a
...
application and custom PostGIS image with the tested Node.js/PostgreSQL releases and pulls the pinned
Caddy release. It does not discover or install an unreviewed image version on the Mac mini.
