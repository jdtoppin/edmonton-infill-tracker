...
- Let the scheduled support-component workflow coordinate stable same-major Node.js, npm, npm bundled-dependency security overrides, Caddy, Caddy's Go build dependencies, and PostgreSQL pins. It may push only an isolated `codex/support-components-*` branch and may open a pull request only after a dispatched full CI run passes; it must never merge or deploy.
