...
export default async function FailuresPage({
  ...
  searchParams: Promise<{ failureId?: string }>;
}) {
  await requireAdmin("/admin/failures");
  const requested = (await searchParams).failureId;
  const db = await getDb();
  const [failures, selected] = await Promise.all([
    db.importFailure.findMany({
  ...
  return (
    <section>
      <PageHeading
        ...
        actions={
          <Badge tone={failures.length > 0 ? "copper" : "green"}>{failures.length} recent</Badge>
        }
      />
      <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
        <div className="overflow-hidden rounded-xl border border-[var(--border)] bg-white">
          {failures.map((failure) => (
            <Link
              key={failure.id}
              href={`/admin/failures?failureId=${encodeURIComponent(failure.id)}`}
              className="block border-b border-[var(--border)] p-4 text-inherit no-underline last:border-0 hover:bg-[#fbfcfa]"
            >
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <span className="mt-1 block text-xs text-[var(--muted)]">
                    {failure.importRun.sourceProvider} · {failure.createdAt.toLocaleString("en-CA")}
                  </span>
                </div>
